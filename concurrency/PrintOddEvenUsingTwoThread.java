...
public class PrintOddEvenUsingTwoThread {
	public static void main(String[] args) {
		...
		MyRunnable r1 = new MyRunnable(true, printer);// isOdd = true..for odd
		Thread t1 = new Thread(r1);
		MyRunnable r2 = new MyRunnable(false, printer);// isOdd = false..for even
...
class MyRunnable implements Runnable {
	...
	public void run() {
		int number = this.isOdd == true ? 1 : 2;
		while (number <= 10) {
			if (isOdd) {
				try {
